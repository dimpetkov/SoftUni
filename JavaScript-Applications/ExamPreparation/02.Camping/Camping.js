class SummerCamp {
    ...
    registerParticipant(name, condition, money) {
        ...
        if(this.listOfParticipants.some(x => x.name == name)) { //check in an object if participant with this name is registered
    ...
    unregisterParticipant(name) {
        let indexOfParticipant = this.listOfParticipants.findIndex(x => x.name == name); //if is not registered will return -1
        if(indexOfParticipant == -1) { // -1 means that is not on the list
